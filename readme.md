...
`windows登录和Mac类似，下载MobaXterm后，打开本地终端，输入如下命令打开主文件夹： open . （注意末尾.）；将ssh文件夹copy到主文件夹，并命名为.ssh (注意ssh前面有.)；如果已经有.ssh，选择合并操作。注意文件夹权限设置！！！`

注意.ssh下的所有文件的权限，一般是600或者644，都可以试一下，有时权限不对会导致链接不上!!!
